...
if(isset($_POST['cpid']) && $_POST['cpid']!='')
{
	...
	$query="insert into sys_jhsj(dhid,mc,dinghl,dj,paix,bz,lury) values(".$_POST['dhid'].",".$_POST['cpid'].",".$_POST['dinghl'].",".$_POST['dj'].",".$paix.",".$_POST['bz'].",'".$_SESSION['uname']."')";
	include("./inc/xexec.php");
}
if(isset($_POST['row']) && $_POST['delrow']==0)
{
	...
	{
		if($_POST['paix'][$i]!=$_POST['opaix'][$i] || $_POST['songhl'][$i]!=$_POST['osonghl'][$i] || $_POST['dinghl'][$i]!=$_POST['odinghl'][$i] || $_POST['shisl'][$i]!=$_POST['oshisl'][$i] || $_POST['danj'][$i]!=$_POST['odanj'][$i] || $_POST['bz'][$i]!=$_POST['obz'][$i])
		{
		...
		$query="update sys_jhsj set paix=".$_POST['paix'][$i].",dinghl=".$_POST['dinghl'][$i].",songhl=".$_POST['songhl'][$i].",shisl=".$_POST['shisl'][$i].",dj=".$_POST['danj'][$i].",shisje=".$_POST['shisje'][$i].",bz=".$_POST['bz'][$i]." where id=".$_POST['id'][$i];
...
$row=0;
$query = "select sj.id,cp.bh,cp.mc,cp.gg,cast(sj.dinghl as varchar),sj.fudw,sj.songhl,sj.shisl,cp.dw,sj.dj,sj.shisje,sj.bz,dh.zt,sj.paix,cp.huansz,sj.mc from sys_jhsj sj,sys_cp cp,sys_jhdh dh where dh.id=sj.dhid and sj.mc=cp.id and sj.dhid=".$_REQUEST['dhid']." order by sj.paix";
$result=sqlsrv_query($conn,$query);
...
?>
	...
	<tr onclick="k(this)" onMouseOver="v(this)" onMouseOut="o(this)">
		...
		<td width="4%"><input type="hidden" name="opaix[<?php echo $row;?>]" value="<?php echo $line[13];?>"><input onfocus="this.select();" id="<?php echo 500+$row;?>" name="paix[<?php echo $row;?>]" value="<?php echo $line[13];?>" style="text-align:center;height:100%;width:100%;background-color: #EAEAEA;"></td>
		<td width="10%" align="center"><?php echo $line[1];?></td>
		<td width="16%"><?php echo $line[2];?></td>
		...
		<td width="7%"><input type="hidden" name="oshisl[<?php echo $row;?>]" value="<?php echo $line[7];?>"><input onfocus="this.select();" id="<?php echo 2500+$row;?>" name="shisl[<?php echo $row;?>]" value="<?php echo $line[7];?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
		<td width="5%" align=center><?php echo $line[8];?></td> 
		<td width="7%"><input type="hidden" name="odanj[<?php echo $row;?>]" value="<?php echo $line[9];?>"><input onfocus="this.select();" id="<?php echo 3000+$row;?>" name="danj[<?php echo $row;?>]" value="<?php echo $line[9];?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
		<td width="8%" align=right><?php echo $line[10];?></td>
		<td width="13%"><input type="hidden" name="obz[<?php echo $row;?>]" value="<?php echo $line[11];?>"><input onfocus="this.select();" id="<?php echo 4500+$row;?>" name="bz[<?php echo $row;?>]" value="<?php echo $line[11];?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
