...
if(isset($_POST['dt1']))
{
	...
	$query="insert into sys_selljgsj(dhid,cpid,sellprice,beiz) select ".$dan.",cpid,sellprice,beiz from sys_selljgsj where dhid=".$_REQUEST['dhid'];
	include("./inc/xexec.php");
	if($res)
	{
...
{
	$query="select b.fenlmc,a.id,a.dh,CONVERT(varchar(10),a.brq,120),CONVERT(varchar(10),a.erq,120),case len(a.beiz) when 0 then null else a.beiz end,a.unitid from sys_selljg a,sys_khfenl b where a.unitid=b.id and a.id=".$_REQUEST['dhid'];
	$result=sqlsrv_query($conn,$query);
	$line=sqlsrv_fetch_array($result);
	$DH=$line[2];
	$BRQ=$line[3];
...
<script language="javascript" src="xSelkhflMohu.js"></script>
</head>
<BODY >
...
<tr><td align=center height="40"><span class="c-red">*</span>客户分类:<input type="hidden" value="<?php echo $UNIT;?>" name="khflid"><input type="hidden" id="oldvalue" name="oldvalue"><input type="text" class="input-text" onkeyup="AutoFinish()" value="<?php echo $UNITmc;?>" title="请输入关键字"  onclick="this.select();CloseTipDiv();" onkeydown="if(event.keyCode==13) startRequest(this.value,0)" style="width: 210px;"  id="spdm" name="spdm"></td></tr>
...
function startRequest(spbh,id)//如果传产品id进来直接得到，否则模糊搜索
{
	...
	xmlHttp.open("post","Cs_SellfljgAddAjax.php",true);//提交返回结果的php页面
