...
{
	$query="update sys_gysfenl set yn=yn^1 where id=".$_POST['delrow'];
	require('./inc/xexec.php');
}
if(isset($_POST['fenlmc']) || $_POST['fenlmc']!="")
{
	$query="insert into sys_gysfenl(bianh,fenlmc,yn) values('".$_POST['bianh']."','".$_POST['fenlmc']."',1)";
	require("./inc/xexec.php");
}
$ed_row=0;
if(isset($_POST['edtrow']) and $_POST['edtrow']!=0)
{
	...
	{
	$query="update sys_gysfenl set bianh='".$_POST['bianh_']."',fenlmc='".$_POST['fenlmc_']."' where id=".$_POST['edtrow'];
	require("./inc/xexec.php");	
	$ed_row=0;
	}
...
$query="select typeb,count(*) from sys_unit where yn=1 and mode=1 group by typeb";//每个分类包含的客户数
$result=sqlsrv_query($conn,$query);
while($line=sqlsrv_fetch_array($result))
	$dat[$line[0]]=$line[1];
sqlsrv_free_stmt($result);

$query="select 0,lx.id,lx.bianh,lx.fenlmc,case lx.yn when 1 then '启用' else '<font color=gray>停用' end from sys_gysfenl lx where lx.id>0 ".$TJ." order by lx.bianh";
$result=sqlsrv_query($conn,$query);
$row=0;
while($line=sqlsrv_fetch_array($result))
{
...
<?php 
	...
	{
	$aquery="select 1,usercode+shortname from sys_unit where yn=1 and mode=1 and typeb=".$line[1];
