...
$ckid = implode(',',$ckid);
$query="
select top 1 sj.ck from sys_jhsj as sj
LEFT JOIN sys_jhdh as dh ON sj.dhid=dh.id
LEFT JOIN sys_cangk as ck ON sj.ck=ck.id
LEFT JOIN sys_cp as cp ON sj.mc=cp.id
WHERE dh.lury='".$_SESSION['uname']."' and sj.ck IN(".$ckid.")
ORDER BY sj.id DESC
";
$result=sqlsrv_query($conn,$query);$line=sqlsrv_fetch_array($result);$maxck=$line['ck'];
$query="select id,fenlmc from sys_cangk where yn=1";
$result=sqlsrv_query($conn,$query);
...
<td algin=center><div class="text-c">盘点数</div></td>
...
<a class="btn radius" href="javascript:{openwindow2(\'I_chukMxPrint.php?dhid='.$dhid.'&p=3\',700,500);}"><img border="0" src="im/dy.png">打印盘点</a>
</span>';
$lie=",序,编号,商品名称,单位,规格,仓库,盘点量,单价,金额,备注,删除,修改";
...
function sub()
{
	if(window.IFrm.cpid.value=="")
		...
		layer.msg('价格不能为空,系统如没自动提取请查询采购价录入!');
	else
	{
		window.IFrm.submit();
		window.IFrm.reset();
