...
$query="select unit from sys_jhdh where id=".$_POST['dhid'];
...
if(isset($_POST['shsid']) and $_POST['shsid']!=0)//一键匹配送货数=配送数
{
    $query="update sys_jhsj set songhs=dinghs,shisje=dinghl*dj where dhid=".$_POST['shsid'];
    include("./inc/xexec.php");
}
if(isset($_POST['sssid']) and $_POST['sssid']!=0)//一键匹配实收数=配送数
{
    $query="update sys_jhsj set shiss=songhs,shisje=dinghl*dj where dhid=".$_POST['sssid'];
    include("./inc/xexec.php");
}
$delrow=0;
if(isset($_POST['cpid']) && $_POST['cpid']!='')
{
    ...
    sqlsrv_free_stmt($result);
    	if($_POST['dinghl']!=0 && $_POST['dinghs']!=0)
		echo "<script language=\"JavaScript\">alert(\"不能同时输入订货量和订货数\")</script>";
	else
	{
        //判断预定数量是不是为空
        	if($_POST['dinghs']!=0)
		{
            	$dinghl=$_POST['dinghs']*$_POST['chg'];
            	$query="insert into sys_jhsj(dhid,mc,dinghl,dinghs,khpmid,dj,shisje,paix,bz,lury,chg) values(".$_POST['dhid'].",".$_POST['cpid'].",".$dinghl.",".$_POST['dinghs'].",".$_POST['khpmid'].",".$_POST['dj'].",".$_POST['dj']."*".$_POST['dinghs'].",".$paix.",".$_POST['bz'].",'".$_SESSION['uname']."',".$_POST['chg'].")";
            	include("./inc/xexec.php");
        	}
		else
		{
            	$query="insert into sys_jhsj(dhid,mc,dinghl,khpmid,dj,shisje,paix,bz,lury,chg) values(".$_POST['dhid'].",".$_POST['cpid'].",".$_POST['dinghl'].",".$_POST['khpmid'].",".$_POST['dj'].",".$_POST['dj']."*".$_POST['dinghl'].",".$paix.",".$_POST['bz'].",'".$_SESSION['uname']."',".$_POST['chg'].")";
            	include("./inc/xexec.php");
        	}
    	}
}
if(isset($_POST['row']) && $_POST['delrow']==0)
{
    ...
    {
        if($_POST['paix'][$i]!=$_POST['opaix'][$i] || $_POST['songhl'][$i]!=$_POST['osonghl'][$i] || $_POST['dinghl'][$i]!=$_POST['odinghl'][$i] || $_POST['shisl'][$i]!=$_POST['oshisl'][$i] || $_POST['danj'][$i]!=$_POST['odanj'][$i] || $_POST['bz'][$i]!=$_POST['obz'][$i] || $_POST['songhs'][$i]!=$_POST['osonghs'][$i] || $_POST['shiss'][$i]!=$_POST['oshiss'][$i] || $_POST['dinghs'][$i]!=$_POST['odinghs'][$i])
        {
            $_POST['bz1'][$i]=$_POST['bz1'][$i]==""?"null":"'".$_POST['bz1'][$i]."'";
            ...
            $query="update sys_jhsj set paix=".$_POST['paix'][$i].",fudw=".$_POST['fudw'][$i].",dinghl=".$_POST['dinghl'][$i].",songhl=".$_POST['songhl'][$i].",shisl=".$_POST['shisl'][$i].",dj=".$_POST['danj'][$i].",shisje=".$_POST['shisje'][$i].",bz=".$_POST['bz'][$i].",bz1=".$_POST['bz1'][$i]." where id=".$_POST['id'][$i];
...
<form action="" method=post name="Frm">
    ...
    <table border="1" class="tableborder3">
        ...
        $menuright=menuright(14);//取得菜单权限
        $row=0;
        //$query = "select sj.id,cp.bh,cp.mc,cp.gg,cast(sj.dinghl as varchar),sj.fudw,cast(sj.songhl as varchar),cast(sj.shisl as varchar),cp.dw,sj.dj,sj.shisje,sj.bz,dh.zt,sj.paix,cp.huansz,sj.mc,sj.bz1,sj.dinghs,khpm.huans,sj.songhs,sj.shiss  from sys_jhsj sj,sys_cp cp,sys_jhdh dh,sys_khpm khpm where dh.id=sj.dhid and khpm.cpid=cp.id and khpm.khid=dh.unit and sj.mc=cp.id and sj.dhid=".$_REQUEST['dhid']." order by sj.paix";
		$query = "select sj.id,cp.bh,cp.mc,cp.gg,cast(sj.dinghl as varchar),case when khpm.gg is null then khpm.dw else isnull(khpm.gg,'')+'/'+isnull(khpm.dw,'') end,cast(sj.songhl as varchar),cast(sj.shisl as varchar),cp.dw,sj.dj,sj.shisje,sj.bz,dh.zt,sj.paix,cp.huansz,sj.mc,isnull(khpm.bh,'')+khpm.mc,sj.dinghs,khpm.huans,sj.songhs,sj.shiss from sys_jhsj sj,sys_cp cp,sys_jhdh dh,sys_khpm khpm,sys_unit unit where dh.id=sj.dhid and sj.khpmid=khpm.id and dh.unit=unit.id and sj.mc=cp.id and sj.dhid=".$_REQUEST['dhid']." order by sj.paix";
        $result=sqlsrv_query($conn,$query);
        while($line=sqlsrv_fetch_array($result))
        {
            ...
            if($menuright>1 and $line[12]==0)//录入
            {
                ...
                <tr onclick="k(this)" onMouseOver="v(this)" onMouseOut="o(this)">
                    ...
                    <td width="13%"><?php echo $line[2];?></td>
                    <td width="3%" align=center><?php echo $line[8];?></td>
                    <td width="14%"><?php echo $line[16];?></td>
                    <td width="5%"><?php echo $line[5];?></td>

                    <td width="4%"><input type="hidden" name="odinghs[<?php echo $row;?>]" value="<?php echo $line[4];?>"><input onfocus="this.select();" id="<?php echo 800+$row;?>" name="dinghs[<?php echo $row;?>]" value="<?php echo $line[17]?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
                    <td width="7%"><input type="hidden" name="odinghl[<?php echo $row;?>]" value="<?php echo $line[4];?>"><input onfocus="this.select();" id="<?php echo 1000+$row;?>" name="dinghl[<?php echo $row;?>]" value="<?php echo $line[4]; ?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>

                    <td width="4%"><input type="hidden" name="osonghs[<?php echo $row;?>]" value="<?php echo $line[6];?>"><input onfocus="this.select();" id="<?php echo 1700+$row;?>" name="songhs[<?php echo $row;?>]" value="<?php echo $line[19];?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
                    ...
                    <td width="4%"><input type="hidden" name="oshiss[<?php echo $row;?>]" value="<?php echo $line[7];?>"><input onfocus="this.select();" id="<?php echo 2200+$row;?>" name="shiss[<?php echo $row;?>]" value="<?php echo $line[20]?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
                    ...
                    <td width="10%"><input type="hidden" name="obz[<?php echo $row;?>]" value="<?php echo $line[11];?>"><input onfocus="this.select();" id="<?php echo 6000+$row;?>" name="bz[<?php echo $row;?>]" value="<?php echo $line[11];?>" style="height:100%;width:100%;text-align:center;background-color: #EAEAEA;"></td>
