...
$query="select typee,count(*) from sys_unit where yn=1 and mode=2 group by typee";//每个分类包含的客户数
$result=sqlsrv_query($conn,$query);
while($line=sqlsrv_fetch_array($result))
	$dat[$line[0]]=$line[1];
sqlsrv_free_stmt($result);

$query="select 0,xl.id,xl.bianh,xl.fenlmc,case xl.lx when 1 then '账套' else '<font color=blue>对账</font>' end,case xl.yn when 1 then '启用' else '<font color=gray>停用' end,xl.lx from sys_cwfenl xl where xl.id>0 ".$TJ." order by xl.bianh";
$result=sqlsrv_query($conn,$query);
$row=0;
while($line=sqlsrv_fetch_array($result))
{
	...
	if($ed_row==$line[1])
	{
$jieslx='<select class="select-box" style="width:85px;height:31px;text-align:center;" name="lx"><option value="1" $line[6]=="1"?"selected":"">账套</option><option value="2" $line[6]=="2"?"selected":"">对账</option></select>';
?>
