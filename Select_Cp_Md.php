...
<body margin="0" style="width:100%; height:420;">
<table align="center" style="width:100%; height:530;position:absolute" cellspacing="0" cellpadding="0" border="0">
...
<tr>
<td align=center>
	<select class="select" style="height:30px;width:275px;" id="dafl" name="dafl" onchange="ListLeft()">
	<option value="" style="text-align: center;">一级分类选取</option>
	<?php 
		$query="select cpfenl.id,cpfenl.fenlmc from sys_cpdfl cpfenl where cpfenl.yn=1 order by cpfenl.bianh,cpfenl.fenlmc";
...
<tr>
<td align=center>
	<select class="select" style="height:30px;width:275px;" id="xiaofl" name="xiaofl" onchange="ListLeft()">
	<option value="" style="text-align: center;">二级分类选取</option>
	<?php 
		$query="select cpfenl.id,cpfenl.fenlmc from sys_cpxfl cpfenl where cpfenl.yn=1 order by cpfenl.bianh,cpfenl.fenlmc";
		$result=sqlsrv_query($conn,$query);
		while($line=sqlsrv_fetch_array($result))
		{
			echo '<option value="',$line[0],'">',$line[1],'</option>';
...
<tr>
<td align=center>
	<input class="input-text" id="cxtj"  tabindex="1" name="cxtj"  onkeydown="if(event.keyCode==13) ListLeft()" style="width: 210px;font-family: 微软雅黑; font-size: 12px; line-height: 15px;border: 1px #000000 solid">
...
<td align=center>
	待选列表<br>
	<select style="width:275px;height:330px" name="fromBox" onDblClick="LtoR_S()" id="fromBox" size="18" multiple="multiple">
	<?php 
		$query="select top 200 cp.id,cp.mc from sys_cp cp where cp.yn=1 order by cp.typec,cp.bh";
		$result=sqlsrv_query($conn,$query);
		while($line=sqlsrv_fetch_array($result))
		{
			echo '<option value="',$line[0],'">',$line[1],'</option>';
...
{
	CreateSelect("<?php echo $xiam;?>",cwho,"selid="+IdStr+"&cxtj="+document.getElementById('cxtj').value+"&dafl="+document.getElementById('dafl').value+"&xiaofl="+document.getElementById('xiaofl').value);//第一个参数是ajax取值的php页面系统自动填好,第二个参数为0表示显示左边选择框,为1显示右边为2左右都显示,第三个参数为提交的查询条件
}
function SelOk()
{
	...
	if(s_id=="") s_name="产品选取";
	parent.Frm.cpid.value=s_id;
	parent.Frm.cpmc.value=s_name;
	parent.Frm.cpmc.title=s_name;
