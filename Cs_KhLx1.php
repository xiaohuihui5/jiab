...
$query="select typea,count(*) from sys_unit where yn=1 and mode=2 group by typea";//每个分类包含的客户数
$result=sqlsrv_query($conn,$query);
while($line=sqlsrv_fetch_array($result))
	$dat[$line[0]]=$line[1];
sqlsrv_free_stmt($result);

$query="select 0,lx.id,lx.bianh,lx.fenlmc,case lx.yn when 1 then '启用' else '<font color=gray>停用' end from sys_khleix lx where lx.id>0 ".$TJ." order by lx.bianh";
$result=sqlsrv_query($conn,$query);
$row=0;
while($line=sqlsrv_fetch_array($result))
{
