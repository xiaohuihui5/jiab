...
$lnk='<span class="l"><a href="Cs_Sellfljg.php"  class="btn radius"><img border=0 src=im/zhuy.png> 按分类设价</a> <a href="Cs_Sellfdjg.php"  class="btn radius"> 按客户设价</a> <a href="Cs_Sellfdtj.php"  class="btn radius"> 按客户特价</a></span>';
$lnk.='<span class="r">';
if($menuright>1)//录入
...
if($menuright>1)//区分权限
{
	$lie=",序,单号,制单日期,客户分类名称,作用起日,结束日期,制单,备注,复,改,删";
	$wid=",4%,11%,10%,20%,10%,10%,6%,16%,5%,4%,4%";
}
else
{
	$lie=",序,单号,制单日期,客户分类名称,作用起日,结束日期,制单人,备注";
...
function Add()
{
	layer_show2('新增分类报价表','<?php echo $xiam;?>Add.php','500','500'); //最后一个参数是给一个标识符 
} 
</script>
