...
$tit='采购管理 <span class="c-gray en">&gt;</span> 采购录入';
...
$lnk='<span class="l"><a href="'.$xiam.'.php"  class="btn radius"><img border=0 src=im/zhuy.png> 采购订单</a></span>';
$lnk.='<span class="r">';
if($menuright>2)//审核
...
$lnk.='<a onclick="Add()" class="btn radius"><img border=0 src=im/add.png> 新增采购单[F9]</a> '; 
...
if($menuright>1)//区分权限
{
	$lie=',序,单号,供应商名称,备注,到货日期,订货量,到货量,实收量,差量,实收金额,制单人,状态,复,改,删';
	$wid=',4%,10%,16%,16%,9%,6%,6%,6%,4%,6%,5%,3%,3%,3%,3%';
}
else
{
	$lie=',序,单号,供应商名称,备注,到货日期,订货量,到货量,实收量,差量,实收金额,制单人,状态';
...
function Add()
{
	layer_show2('新增采购订单','<?php echo $xiam;?>Add.php','500','500'); //最后一个参数是给一个标识符 
} 
</script>
