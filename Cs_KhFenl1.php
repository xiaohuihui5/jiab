...
{
	$query="update sys_khfenl set yn=yn^1 where id=".$_POST['delrow'];
	require('./inc/xexec.php');
}
if(isset($_POST['fenlmc']) || $_POST['fenlmc']!="")
{
	$query="insert into sys_khfenl(bianh,fenlmc,yn,piny) values('".$_POST['bianh']."','".$_POST['fenlmc']."',1,'".Get_Piny($_POST['fenlmc'])."')";
	require("./inc/xexec.php");
}
$ed_row=0;
if(isset($_POST['edtrow']) and $_POST['edtrow']!=0)
{
	...
	{
	$query="update sys_khfenl set bianh='".$_POST['bianh_']."',fenlmc='".$_POST['fenlmc_']."',piny='".Get_Piny($_POST['fenlmc_'])."' where id=".$_POST['edtrow'];
	require("./inc/xexec.php");	
	$ed_row=0;
	}
...
if(isset($_POST['cxtj']) and $_POST['cxtj']!="")
	$TJ=" and fl.fenlmc like '%".$_POST['cxtj']."%' ";

$query="select typeb,count(*) from sys_unit where yn=1 and mode=2 group by typeb";//每个分类包含的客户数
$result=sqlsrv_query($conn,$query);
while($line=sqlsrv_fetch_array($result))
	$dat[$line[0]]=$line[1];
sqlsrv_free_stmt($result);

$query="select 0,fl.id,fl.bianh,fl.fenlmc,case fl.yn when 1 then '启用' else '<font color=gray>停用' end from sys_khfenl fl where fl.id>0 ".$TJ." order by fl.bianh";
$result=sqlsrv_query($conn,$query);
$row=0;
while($line=sqlsrv_fetch_array($result))
{
...
?>
	...
	<td width="10%" align="center"><a href="javascript:mod(<?php echo $line[1];?>,0)"><img border=0 src=im/xiug.png alt=修改此单></td>
	</tr>
<?php 
	}
	if(isset($_POST['selid']) and $_POST['selid']!="" and $line[1]==$_POST['selid'])//列出所包含分类的产品信息
	{
	$aquery="select 1,usercode+shortname from sys_unit where yn=1 and mode=2 and typeb=".$line[1];
