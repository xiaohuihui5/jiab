...
<table style="width:100%; height:100%;" cellSpacing="0" cellPadding="0" border="0" class="table1">
	<tr>
	<td align=center>
			<select class="select" style="height:30px;width:275px;" id="typea" name="typea" onchange="ListLeft()">
			<option value="" style="align:center">-------结算方式-------</option>
	...
	<td align=center>
			...
			<option value="" style="align:center">-------所属供应商分类选取-------</option>
			<?php 
				$query="select fl.id,fl.fenlmc from sys_gysfenl fl where fl.yn=1 order by fl.bianh";
	...
	<tr><td align=center>
	待选列表<br>
	<select style="width:275px;height:310px" name="fromBox" onDblClick="LtoR_S()" id="fromBox" size="18" multiple="multiple">
	<?php 
		$query="select top 2000 unit.id,isnull(unit.usercode,'')+isnull(unit.shortname,'') from sys_unit unit where unit.mode=1 and unit.yn=1 order by unit.usercode";
		$result=sqlsrv_query($conn,$query);
		while($line=sqlsrv_fetch_array($result))
		{
			echo '<option value="',$line[0],'">',$line[1],'</option>';
...
{
	CreateSelect("<?php echo $xiam;?>",cwho,"selid="+IdStr+"&cxtj="+document.getElementById('cxtj').value+"&typea="+document.getElementById('typea').value+"&typeb="+document.getElementById('typeb').value);//第一个参数是ajax取值的php页面系统自动填好,第二个参数为0表示显示左边选择框,为1显示右边为2左右都显示,第三个参数为提交的查询条件
}
function SelOk()
{
	var s_id="";
	...
	if(s_id=="") s_name="供应商选取";
	parent.Frm.gysid.value=s_id;
	parent.Frm.gysmc.value=s_name;
	parent.Frm.gysmc.title=s_name;
