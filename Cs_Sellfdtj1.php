...
if(isset($_POST['khflid']) and $_POST['khflid']!="")
	$TJ.=" and b.typeb in(".$_POST['khflid'].") ";
if(isset($_POST['cxtj']) and $_POST['cxtj']!="")
	$TJ.=" and b.shortname like '%".$_POST['cxtj']."%' ";
...
$macmac="select a.id,0,a.dh,CONVERT(varchar(10),a.dhrq,120),b.shortname,CONVERT(varchar(10),a.brq,120),CONVERT(varchar(10),a.erq,120),a.lury,a.beiz,
...
'<a href=javascript:del('+cast(a.id as varchar(10))+')><img border=0 src=im/shanc.png alt=删除此单></a>' as sc
 from sys_selljg a,sys_unit b where b.id=a.unitid ".$TJ." order by ".$px;
if($menuright>1)//录入权限
{
...
$macmac.="#".",序,单号,制单日期,客户名称,起始日期,结束日期,制单,备注,复制新单,修改,删除";
$macmac.="#按客户分类销售价格表";
$macmac.="#";
$macmac.="#";
...
{	
	window.Frm.scroll.value=document.body.scrollTop;
	layer_show3("修改客户报价表单","<?php echo $xiam;?>Edit.php?dhid="+id,"400","400","parent");//最后一个是给标识符  需要父级打开就给  不然就空
}
function cp(id)
{	
	window.Frm.scroll.value=document.body.scrollTop;
	layer_show3("复制客户报价表单","<?php echo $xiam;?>Copy.php?dhid="+id,"400","400","parent");//最后一个是给标识符  需要父级打开就给  不然就空
}
</script>
