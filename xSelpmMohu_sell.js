...
function createXML2HttpRequest()
{
	if (window.ActiveXObject)
	{
...
mOffsetParent = mOffsetParent.offsetParent;
		...
		div.style.top=mOffsetTop+30+"px";
...
//		div.style.top=document.getElementById("spdm").offsetTop+102+"px";
	...
	function AutoFinish()
	{
		switch (event.keyCode) {
				case 13:
				if(document.getElementById("spdm").value!='')//默认按回车直接取第一行结果
				{
					if(document.getElementById("lstab").rows.length>=1)
					{
					document.getElementById("lstab").rows[1].bgColor="#93CDDD";
					document.getElementById("spdm").value=document.getElementById("lstab").rows[1].cells[0].innerText+document.getElementById("lstab").rows[1].cells[1].innerText;
					document.getElementById("khpmid").value=document.getElementById("lstab").rows[1].cells[5].innerText;
					}
					startRequest(document.getElementById("spdm").value,0,document.getElementById("khpmid").value);
				}
				return false;
				break;
				case 38: //up
					if(now_index<1)
					{
						...
						document.getElementById("khpmid").value=document.getElementById("lstab").rows[now_index].cells[5].innerText;
					}
					else
					{
