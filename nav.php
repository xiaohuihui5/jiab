...
?>
	<dl id="menu-article">
		<dt onclick="dl(this)" style="height:40px;"><i class="Hui-iconfont"><?php echo $arr['icon'];?></i> <?php echo $arr['title'];?></dt>
			...
			<ul>
				<?php 
					...
					while($arr1=sqlsrv_fetch_array($result1)){
						$sql2 = "select a.icon,a.menuid,a.layer,a.title,a.url,a.target from sys_menu a,sys_menuright b where a.top_id=".$arr1['menuid']." and b.empid=".$_SESSION['empid']." and a.menuid=b.menuid and a.display=1 order by a.topmenuid";
				...
				?>
					...
					<dd>
						...
						?>
							<li><a href="<?php echo $arr2['url'];?>" title="<?php echo $arr2['title'];?>" target="mainframe"><?php echo $arr2['title'];?></a></li>
							<?php }?>
						</ul>
					</dd>
				<?php }else{?>
				
					<li><a href="<?php echo $arr1['url'];?>" title="<?php echo $arr1['title'];?>" target="mainframe"><?php echo $arr1['title'];?></a></li>
				<?php }?>
				<?php }?>
			</ul>
		</dd>
