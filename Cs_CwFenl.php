...
$tit='基础资料 <span class="c-gray en">&gt;</span> 财务分类';
if($menuright>1)//录入
	$lur='<div class="text-c">编号:&nbsp;<input type="text" class="input-text" style="width:100px" id="bianh" name="bianh" onkeydown="if(event.keyCode==13){window.Frm.fenlmc.select();}else  if(event.keyCode==39){window.IFrm.fenlmc.select();}">&nbsp;财务分类:&nbsp;<input type="text" class="input-text" style="width:150px"  id="fenlmc" name="fenlmc" onkeydown="if(event.keyCode==13){sub();}">
结账类型:&nbsp;<select  class="select-box" style="width:85px;height:31px;text-align:center;" name="lx"><option value="">结算类型</option><option value="1">账套</option><option value="2">对账单</option></select>&nbsp; <input class="btn btn-primary radius" type="button" value="&nbsp;&nbsp;增加&nbsp;&nbsp;" onclick="sub();">';
else
...
<a href="Cs_CwFenl.php"  class="btn radius"><img border=0 src=im/zhuy.png> 财务分类</a>  
</span>';
...
$cha='';
$lie=",序,编号,财务分类,结算类型,含客户数,禁用,修改";
$wid=",10%,20%,40%,10%,10%,5%,5%";
$tis="";
$xuh='';
$yul='';
...
function sub()
{
	...
	{
		parent.layer.msg('财务名称不得为空', {icon:2,time:1500});
		return false;
	}	
	else if(window.Frm.lx.value==""|| window.Frm.lx.value==null)
	{  
		parent.layer.msg('结算类型不能为空', {icon:2,time:1500});
		return false;
	}
	else
	{
