...
$_SESSION['mac']="select 0,unit.usercode+unit.shortname,sum(dh.zf*sj.dinghl),sum(dh.zf*sj.songhl),sum(dh.zf*sj.shisl),case sum(isnull(sj.shisl,0)-isnull(sj.songhl,0)) when 0 then null else sum(isnull(sj.shisl,0)-isnull(sj.songhl,0)) end,sum(dh.zf*sj.shisje) from sys_jhdh dh,sys_jhsj sj,sys_unit unit,sys_cp cp where dh.id=sj.dhid and dh.unit=unit.id and sj.mc=cp.id $TJ group by unit.typea,unit.usercode,unit.shortname order by ".$px;
$_SESSION['mac'].="#"."1,0,1,1,1,1,1";
$_SESSION['mac'].="#".",50%,10%,10%,10%,10%,10%";
$_SESSION['mac'].="#".",left,right,right,right,right,right,right";
$_SESSION['mac'].="#".",供应商名称,订货量,到货量,实收量,差量,实收金额";
